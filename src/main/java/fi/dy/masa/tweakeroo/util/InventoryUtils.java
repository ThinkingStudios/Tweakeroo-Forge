...
public class InventoryUtils {
    ...
    private static double getArmorAndArmorToughnessValue(ItemStack stack, double base, EquipmentSlot slot) {
        ...
        for (Map.Entry<EntityAttribute, EntityAttributeModifier> entry : attributeModifiers.entries()) {
            EntityAttribute attribute = entry.getKey();
            EntityAttributeModifier modifier = entry.getValue();

            switch (modifier.getOperation()) {
                case ADDITION:
                    total[0] += modifier.getValue();
                    break;
                case MULTIPLY_BASE:
                    total[0] += modifier.getValue() * base;
                    break;
                case MULTIPLY_TOTAL:
                    total[0] += modifier.getValue() * total[0];
                    break;
                default:
                    throw new IllegalStateException("Unexpected value: " + modifier.getOperation());
            }
        }
